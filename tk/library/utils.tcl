...
#	set oldFocus [focus]
	...
	wm withdraw $win
	update idletasks
	set x2 [expr {([ScreenWidth $win] - [winfo reqwidth $win]) / $x \
		- [winfo vrootx $win]}]
	set y2 [expr {([ScreenHeight $win] - [winfo reqheight $win]) / $y \
		- [winfo vrooty $win]}]
...
proc NSUtils::GrabRelease {win} {
	...
	grab release $win
	if {$Priv(grab,$depth,oldGrab) ne ""} {
		if {$Priv(grab,$depth,grabStatus) eq "global"} {
...
proc NSUtils::StringBox {args} {
	...
	foreach text $buttons {
		button $frame.button$n \
			...
			-width 9
		CheckButtonWidth $frame.button$n
		pack $frame.button$n \
...
proc NSUtils::GetInteger {string} {
...
# NSUtils::HasCursor --
#
#	Return 1 if the mouse pointer is over the given window.
#
# Arguments:
#	arg1					about arg1
#
...
proc NSUtils::HasCursor {window {compareTop 0}} {

	set pointerxy [winfo pointerxy $window]
	set window2 [winfo containing {*}$pointerxy]
	if {$window2 eq ""} {
		return 0
	}
	if {$compareTop} {
		set window  [winfo toplevel $window ]
		set window2 [winfo toplevel $window2]
	}
	if {$window2 ne $window} {
		return 0
	}
	return 1
}
...
#
#	Return 1 if any child of the given toplevel (or itself) has the focus.
#
# Arguments:
#	arg1					about arg1
#
...
proc NSUtils::DestroyObjectWithWidget {class oop widget} {
	...
	bind $tag <Destroy> "
		# dbwin \"DestroyObjectWithWidget %W\\n\";
		# foreach tag \[bindtags %W\] {dbwin \\t\$tag\\n};
...
proc NSUtils::IsButton {widget} {
	if {[winfo class $widget] eq "Button"} { return 1 }
	if {[winfo class $widget] eq "TButton"} { return 1 }
	return 0
}
...
proc NSUtils::FocusIn_DefaultButton {widget} {
	...
	if {[IsButton $widget]} {
		...
		if {[$widget cget -default] ne "active"} {
	...
	if {[$button cget -default] ne "active"} {
...
# NSUtils::InvokeButton --
#
#	Simulate mouse click/release in a button.
#
# Arguments:
#	arg1					about arg1
#
...
proc NSUtils::InvokeButton {widget} {

	if {$::UseTile} {
		event generate $widget <<Invoke>>
	} else {
		::tk::ButtonInvoke $widget
	}
...
proc NSUtils::InvokeDefaultButton {widget} {
	...
	foreach child [winfo children $widget] {
		if {[IsButton $child]} {
			if {[$child cget -default] eq "active"} {
				InvokeButton $child
				break
			}
		} else {
...
# NSUtils::CheckButtonWidth --
#
#	I like my buttons to have uniform width. But if the text in a button
#	demands more space (i.e. JAPANESE for "Cancel") let the button be its
#	requested width.
#
# Arguments:
#	arg1					about arg1
...
proc NSUtils::CheckButtonWidth {butt} {

	set widthOpt [$butt cget -width]
	if {$widthOpt eq ""} return

	set widthFixed [winfo reqwidth $butt]
	$butt configure -width ""
	set widthLoose [winfo reqwidth $butt]
	if {$widthFixed < $widthLoose} return

	$butt configure -width $widthOpt
