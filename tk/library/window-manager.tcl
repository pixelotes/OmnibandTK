...
proc NSWindowManager::RequestGeometry {winName geometry} {
	...
	# Ensure part of the titlebar is visible
...
proc NSWindowManager::Setup {winName} {
	...
	if {[Platform unix]} {
		# Unix window managers don't update the transient status sometimes
		# unless the master is mapped and the child isn't.
		set master [wm transient $win]
		if {$master ne ""} {
			ASSERT {[winfo ismapped $master]} "attempting to display transient whose master is hidden"
			ASSERT {![winfo ismapped $win]} "calling Setup on an already-visible window"
...
proc NSWindowManager::Display {winName args} {
	...
	if {[wm state $Priv($winName,win)] eq "normal"} {
...
proc NSWindowManager::Undisplay {winName args} {
	...
	if {[wm state $Priv($winName,win)] ne "normal"} return
